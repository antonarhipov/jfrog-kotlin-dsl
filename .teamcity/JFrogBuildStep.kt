...
class JFrogCLI() : ScriptBuildStep() {
    ...
    constructor(block: JFrogCLI.() -> Unit) : this() {
        ...
        if (command == UPLOAD) {
            scriptContent = """
                JFROG_CLI_OFFER_CONFIG=false jfrog rt u target/artifact-1.0-SNAPSHOT.jar generic-local/artifact/%build.number%/ --build-number=$buildNumber --build-name=$buildName --url=http://localhost:8040/artifactory/ --user=admin --password=%artifactoryPassword%
            """.trimIndent()
        }
